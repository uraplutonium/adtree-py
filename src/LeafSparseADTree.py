...
def importModules(recordModule):
    import LeafSparseADTree
    LeafSparseADTree.Record = __import__(recordModule)
...
class ADNode(object):
    ...
    def __init__(self, startAttributeNum, recordNums):
        ...
        if startAttributeNum != Record.arityLength+1:
            if self.__count >= Rmin:
...
class VaryNode(object):
    ...
    def getADNChild(self, attributeValue):
        '''attributeValue ranges from 1(NOT 0) to the arityList[attributeNum]'''
